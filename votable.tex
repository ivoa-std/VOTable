...
It is a draft document and may be updated, replaced,
or obsoleted by other documents at any time.
It is inappropriate to use IVOA Working Drafts as reference materials or to
...
The byte content for each row consists of the bytes for the BINARY
encoding as described in the previous subsection, followed by zero or
more bytes containing a null-value flags for each cell in that row.
These flags are stored as exactly one bit per table column, and the number
of bytes is the smallest required for this purpose;
an $N$-column table will therefore contain $(N+7)/8$ flag bytes per row.
...
It is recommended, but not required, that unused flag bits are unset,
and that cell values flagged as nulls are represented with zero-valued
bytes.
...
This magic value must be a legal value for the column's data type,
for instance in the case of {\attrval{datatype}{unsignedByte}}
it must be an integer in the range 0--255.
This approach, inherited from FITS, works in the same way for all
of the defined VOTable encodings.
...
to the data itself, and it works differently for the different encodings.
In the \elem{TABLEDATA} encoding
an empty \elem{TD} element signals a null value, and
in the \elem{BINARY2} encoding
a separate null-ness flag is provided for each cell.
The \elem{BINARY} and \elem{FITS} encodings do not support this approach
at all.
It should be noted that when using this approach, unlike with magic values,
the different encodings do not have identical capabilities for
representing data, so that lossless round-tripping between encodings
is not always possible.

Some other subtleties concerning null values should also be mentioned:
\begin{itemize}
\item Individual elements can be marked as null only
      by use of the \elem{VALUES}/\attr{null} mechanism, which operates on a
      per-element basis.  It is not possible to denote a null cell
      of an array-valued column using the magic value approach.
      % Alternatively:
      % There is no way to mark individual elements in
      % the cells of an array-valued column as null.
...
\item In either approach, floating point values not formally marked as nulls
      ...
      For many purposes, the distinction between NaN and null is not
      important, and in such cases use of NaN can be more convenient
      than formally marking cells as null.
\item Combining the the two approaches is not encouraged, however
      the \elem{VALUES} \attr{null} attribute must always be respected
      where present.
\item The {\em logical} datatype has its own arrangements for representing
...
\label{dtd}
The XML Schema [7] defining a VOTable 1.3 document
...
.Version 1.3: MT: Add BINARY2 element
-->
<xs:schema 
...
<xs:annotation><xs:documentation>
    VOTable1.3 is meant to serialize tabular documents in the
