...
among \literalvalue{ICRS}, \literalvalue{eq\_FK5},  \literalvalue{eq\_FK4}, \literalvalue{ecl\_FK4},
    \literalvalue{ecl\_FK5}, \literalvalue{galactic}, \literalvalue{supergalactic},
    \literalvalue{barycentric}, \literalvalue{geo\_app} and a user-defined \literalvalue{xy}
    value. \attr{equinox} is the parameter required to fix the 
    equatorial or ecliptic systems (as e.g. \literalvalue{J2000} as
    the default \literalvalue{eq\_FK5} or \literalvalue{B1950} as the default 
    \literalvalue{eq\_FK4}), and \attr{epoch} specifies the epoch of the positions
    if necessary.
...
the parser does not know about, for example {\literalvalue{httpg://server/file}}.
However, parsers are expected to understand at least the {\literalvalue{file}},
{\literalvalue{http}} and {\literalvalue{ftp}} protocols.
\fi             %%% OLD
...
\literalvalue{5.12} and one that is written \literalvalue{5.1200}, in that
...
%to \literalvalue{null}s; superfluous {\elem{TD}}'s are ignored.
