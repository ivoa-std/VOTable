...
\date{2012-10-15 (Volute $\mbox{}$Revision$\mbox{}$)}
...
For example, \attrval{content-role}{doc} indicates documentation,
and in this case dereferencing the URI should yield a document,
suitable for presentation to the user, describing the LINK's parent element.
On the other hand \attrval{content-role}{type} indicates a type-like
relationship between the URI and the LINK's parent,
and in this case the type is named by the URI string itself,
while the content retrieved by dereferencing it, if any, is secondary.
Beyond these two examples, the values and semantics of
the \attr{content-role} attribute are to be defined outside of this
...
In addition the the \elem{LINK} element
...
Note however that this may be overridden
by metadata received during the retrieval operation
(e.g.\ the HTTP Content-Type header).
...
described in \Aref{sec:datatypes}{section 6}, and may never
...
\begin{itemize}
\item Individual elements of an array-valued cell can be marked as null only
...
\item The representation of STC information in \Aref{example1}{section 3.1}
      ...
      {\em STC in VOTable} Note [8].  This usage is recommended even for
