...
% adopted for the version 1.1 of the VOTable format, 
...
This document has been produced by the VOTable Working Group.
...
other documents at any time. It is inappropriate to use IVOA Working Drafts
as reference materials or to cite them as other than {\em``work in progress''}.
\fi
...
\else
%This proposed recommendation is made available for public review.
%Comments to this document should be sent to 
%\A{mailto:votable@ivoa.net}{votable@ivoa.net}, a mailing list with a 
%\A{http://ivoa.net/forum/votable/}{public archive}.
%It is appropriate to reference this document only as a recommended
%standard that is under review and which may be changed
%before it is accepted as a full recommendation.
%
%A list of current IVOA Recommendations and other technical documents
%can be found at\\
%\A{http://ivoa.net/Documents/}{http://ivoa.net/Documents/}
...
%recommendation. A list of {current IVOA Recommendations 
...
%Alternatively, the metadata can be sent
...
provided by XSchema [7] rather than DTD -- a draft version of the
...
and the values of the attributes typed in \value{magenta}.
...
\iffalse        % modified{1.2-090606}
The string in the \attr{arraysize} attribute expresses these
dimensions, each integer except the last being followed by the 
{\tt x} character.
The last (slowest-varying) subscript of a
...
which case the array may be arbitarily large 
(in practice up to $\simeq 2\times10^9$ elements); or a number followed by
...
collection of substrings, and VOTable 1.2 does not support this
{\em(see however the \Aref{sec:arraystring}{appendix A3} which
...
\iffalse
According to the XML standard, the contents of the {\attr{ID}} attribute 
...
%and celestial coordinate system definitions
%described by the elements 
%\elem{PARAM}, \elem{INFO} and \elem{COOSYS} respectively.
...
\elem{COOSYS} element has to be referred to via the \attr{ref} attribute
...
the \elem{COOSYS} may be deprecated in the  future
in favor of a more generic way of describing the conventions used to define
the positions of the objects studied in the enclosed tables.
\fi
...
%with all the required accuracy
...
introduced by a \value{glu:} value of the \attr{href} attribute:
...
In the first example, the sequence {\tt \&\#231;} is interpreted as
part of the Unicode code point, %ISO/IEC 10646 character set, 
...
would be encoded as {\tt New\&\#x20;York}. 
...
\begin{tabular}{ll}
{\value{phot.mag;em.opt.B}}        &  Blue magnitude \\
{\value{src.orbital.eccentricity}} &  Orbital eccentricity \\
{\value{time.period;stat.median}}  &  Median Value of the Period \\
{\value{instr.det.qe}}             &  Detector's Quantum Efficiency \\
\end{tabular}
...
data types.
%\Arefs{sec:datatypes}{Section 6} indicates the default {\em null} values for 
%each of the primitive data types when the \elem{TABLEDATA} data representation
%is being used. 
Some of the primitive data types have one or more representations of the
{\em null} value %{\em null} values defined
(for the \value{char}, \value{float} and \value{double}
types an empty cell may be used). Other types (\value{boolean}, 
\value{unsignedByte}, \value{short}, and \value{int}) have no default
...
%(via the \elem{FIELD} element), or
%%(contains a \attr{datatype} field), or 
%{\em logically}  %(via  a \elem{FIELDref} element)
%by means of a \elem{FIELDref} element which references a field 
%defined in the same parent \elem{TABLE}.
...
%is obtained by replacing the definitions of columns 4 and 5 by 
...
%\else\begingroup\small\fi
\begin{verbatim}
    <GROUP name="Velocity" ucd="src.veloc.hc">
      <DESCRIPTION>Velocity and its error</DESCRIPTION>
      <FIELD name="RVel" ID="col4" ucd="src.veloc.hc" datatype="float" 
             width="5" unit="km/s"/>
      <FIELD name="e_RVel" ID="col5" ucd="stat.error;src.veloc.hc" 
             datatype="float" width="3" unit="km/s"/>
    </GROUP>
\end{verbatim}
%\ifhtx\End{tabular}\else\endgroup\fi

\noindent A {\em logical} definition of this group could alternatively be
...
%%%%%%%%%%%% Removed follwing Tony's comments
%tags having a \attr{datatype} attribute, with each row 
...
\label{Image1}
\ifhtx\tag{IMG SRC="serial.gif" NAME="Image1" ALIGN=BOTTOM BORDER=0}\br
\else
\begin{figure}[hbt]
...
%and is the only serialization method that does not allow an 
%encoding or a remote data stream. 
...
%%%%%%%% Modified following Tony's comments
contains a value which must be compatible with 
%is passed to a reader that is implicitly defined by 
...
%cell will contain that value, and is therefore assumed to contain no data.
...
the white space is significant for \value{char} or
\value{unicodeChar} datatypes, and for instance
...
finally, \value{file} is a reference to a local file.
VOTable parsers are not required to understand arbitrary protocols,
...
\value{a} to \value{f} (i.e. a digit in a hexadecimal representation
of a number).
%; in this section we write the hexadecimal representation
%as {\bf 0x} followed by hexadigits.
...
or \verb+&#x3C;+ and \verb+&#x3E;+.
\iffalse
and a blank which would be interpreted
...
number. If either member contains a \value{NaN},
...
%\end{quote}
%
%\noindent The XML Schema represents the actual definition of
%a VOTable document. 
...
%quite generally presented in a structured interpreted in the browser, and a 
...
%\item  a clarification of the {\em null} possibilities
%       (see \Aref{sec:datatypes}{datatypes} section)
...
%\\ \hspace*{2em}{{http://xml.gsfc.nasa.gov/XDF/XDF\_home.html}}
...
\iffalse
\subsection{Additional {\tt TABLE} Attributes}  % Suggested TomMcGlynn
