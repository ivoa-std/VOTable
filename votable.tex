...
% \date{2013-08-06 (Volute $\mbox{}$Revision$\mbox{}$)}
\date{2019-01-31}
...
\urlthisversion{%\par\noindent {\bf This version:}\\
  %WD-VOTable-1.4-20190131
  \url{http://www.ivoa.net/Documents/VOTable/20190131/}} 
...
\normalsize \twikiname{MarkusDemleitner}{Markus {\bf Demleitner}}
        ...
        \normalsize \twikiname{TomDonaldson}{Tom {\bf Donaldson}}
...
may include \attr{schemaLocation} attribute, consumers are not required
to honor it.  
...
a \elem{RESOURCE} (\Aref{elem:RESOURCE}{section 3.6})
or a \elem{TABLE} (\Aref{elem:TABLE}{section 3.8}) are better placed 
...
\subsection{\texorpdfstring{\elem{COOSYS} Element}
	{COOSYS Element}}
\label{elem:COOSYS}
...
\subsection{\texorpdfstring{\elem{TIMESYS} Element}
	{TIMESYS Element}}
\label{elem:TIMESYS}

The \elem{TIMESYS} element (introduced in VOTable 1.4) defines metadata
for temporal coordinates.  As with \elem{COOSYS}, \elem{FIELD}s (and
possibly \elem{PARAM}s) SHOULD reference the \elem{TIMESYS} giving their
frame using the VOTable \attr{ref} attribute; absent such a \attr{ref},
readers SHOULD assume the lexically first \elem{TIMESYS} element in the
VOTable to be pertinent for time-like quantities. A \elem{TIMESYS} element referenced via a \attr{ref} attribute
...
In the example, the \elem{TIMESYS} element describes that time system.  The \elem{TIMESYS} ID value needs to be unique within the document so that it can be referenced by  \elem{FIELD}s or  \elem{PARAM}s.  Then the \attr{obs\_time} \elem{FIELD} indicates that its values should be interpreted in that time system by referring back to the \elem{TIMESYS} element using \attrval{ref}{time\_frame}.

Similarly, the \elem{COOSYS} element defines the coordinate system, and is referred to by the  \attr{ra} 
and  \attr{dec} \elem{PARAM} elements.  Note that since the sky position is defined by \elem{PARAM}s instead 
of \elem{FIELD}s, the same sky position applies to each row of the \elem{TABLE} without the values appearing in 
\elem{TD} elements.

Further (non-normative) information on best practices and usage patterns
for \elem{TIMESYS} can be found in [13].
...
existing within a \elem{RESOURCE} element (see \Aref{sec:link}{section 3.7}).
...
\item   The \attr{arraysize} attribute exists when 
        ...
        (see also \Aref{sec:dim}{section 2.2}).
...
(DALI, see [8]), defines the use of \attr{xtype} for various use cases.
...
The XML Schema [6] defining a VOTable 1.4 document
...
\item In view of the new options for flagging null values introduced by the
      ...
      in favour of NaN.
\item The description of the \elem{LINK} element (\Aref{sec:link}{section 3.7})
...
\noindent [3] {\it Units in the VO}
\\ \hspace*{2em}{\url{http://www.ivoa.net/documents/latest/VOUnits.html}}
...
\\ \hspace*{2em}{\url{http://ivoa.net/documents/Notes/XMLVers/}}
\par
\noindent [8] {\it Data Access Layer Interface}
\\ \hspace*{2em}{\url{http://www.ivoa.net/documents/DALI/20170517/REC-DALI-1.1.pdf}}
...
\par
\noindent [13] {\it A Proposal for a TIMESYS Element in VOTable}
\\ \hspace*{2em}{\url{http://ivoa.net/documents/Notes/TimeSys/}}
...
\ifhtx
\W{xsd}{http://www.ivoa.net/xml/VOTable/votable-1.4.xsd}
