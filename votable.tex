...
serialization as described in the previous subsection, followed by zero or
more bytes containing a null-value flag for each cell in that row.
...
Unused bits will be at the less-significant bits of the final flag byte,
and shall be unset (0).

It is recommended, but not required, that cell values flagged as nulls
are filled with with zero-valued bytes.
...
\caption{\label{fig:bin}Data Storage in BINARY2 mode}
\end{figure}\end{center}
\fi
