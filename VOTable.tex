...
\previousversion[https://www.ivoa.net/documents/VOTable/20250116/]
                {https://www.ivoa.net/documents/VOTable/20250116/
                 (V1.5 2025-01-16)}
...
\previousversion[http://www.ivoa.net/documents/VOTable/20091130/]
                {http://www.ivoa.net/documents/VOTable/20091130/
                 (V1.2 2009-11-30)}
\previousversion[http://www.ivoa.net/documents/VOTable/20040811/]
                {http://www.ivoa.net/documents/VOTable/20040811/
                 (V1.1 2004-08-11)}
...
To encode or decode a multi-dimensional {\tt char} array
it is necessary first to represent it as a sequence of UTF-8 bytes
in order to determine the byte sequence representing
each fixed-storage-length string.
...
This document defines four values for the \attr{content-role} attribute:
...
\item \attrval{content-role}{hints}	indicates a pointer to information to be 
      used by use by the application.

\item \attrval{content-role}{query}	indicates a query URL in the context of a 
      query extension. In this case, \attr{content-role}
	  must be used together with an {\attr{action}} attribute (see \ref{query}).
...
\item   {\fg{black}}The \attr{precision} and \attr{width} attributes
        characterise the numerical accuracy and display size
        of the data
        (see \Aref{sec:form}).
...
\begin{itemize}
\item   The \attr{value} attribute provides the \elem{PARAM}eter's value,
        encoded in the same way as for the \elem{TABLEDATA} serialization
        (see \Aref{sec:datatypes}).
        \attr{value} is a required attribute of the \elem{PARAM} element.
\end{itemize}

\subsection{The \attr{precision} and \attr{width} Attributes}
\label{sec:form}
...
\item   The {\attr{width}} attribute %of the {\elem{FIELD}},
        ...
        the number of characters to be used for display of the quantity.
        This has special significance when used with character data,
        see below.
\end{itemize}
...
\begin{itemize}
        ...
        This occurs with the http protocol, where the headers indicate
...
In the \elem{BINARY}/\elem{BINARY2} serialization
...
but a string having the number of bytes identical to
...
\item {\bf Unicode Character}
\emph{(\textbf{deprecated} from VOTable 1.6)}\quad
If the value of the {\attr{datatype}}
...
A VOTable document should be introduced by a Media Type
(sometimes called MIME type).
Media type syntax is described in RFC 2045 section 5.1, and
its semantics in RFC 2046.
Associating a media type to a document enables the {\em data consumer}
(an application or a web browser) to launch the desired application
({\em e.g.} a visualisation tool).

In the HTTP protocol (RFC 9110), the media type is the value specified by the
{\textsf{Content-Type:}} header. The recommended media type describing
...
this parameter, and that VOTables are encoded using UTF-8 without a
Byte Order Mark (BOM).
\end{description}

Alternatively the {\literalvalue{\textsf{text/xml}}} media type is acceptable
for services delivering data which are expected to be
visualized by humans in a browser; this media type
...
media type with the same comments as above.
...
\item Appendix A.1 (VOTable LINK substitutions) has been removed
